...
const main = async () => {
  ...
  await run("git push");
  await run("pnpm --filter astro-variables build");
  if (values.otp) {
  ...
  await run(
    `gh release create ${tag} --title ${tag} --notes "Please refer to [CHANGELOG.md](https://github.com/varharrie/astro-variables/blob/main/package/CHANGELOG.md) for details."`
  );
};
